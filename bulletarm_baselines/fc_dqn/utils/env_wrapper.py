...
class EnvWrapper:
    ...
    def _resetAttack(self, position):
        (states, in_hands, obs, object_dir_list, object_param_list) = self.envs._resetAttack(position)
        states = torch.tensor(states).float()
        in_hands = torch.tensor(in_hands).float()
        obs = torch.tensor(obs).float()
        # object_param_list = [torch.tensor(param) for param in object_param_list]
    ...
    def stepAttack(self, actions, auto_reset=False):
        actions = actions.cpu().numpy()
        (states_, in_hands_, obs_), rewards, dones = self.envs.stepAttack(actions, auto_reset)
        states_ = torch.tensor(states_).float()
        in_hands_ = torch.tensor(in_hands_).float()
        obs_ = torch.tensor(obs_).float()
        rewards = torch.tensor(rewards).float()
