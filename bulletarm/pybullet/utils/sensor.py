...
class Sensor(object):
  ...
  def importSingleObject(self, scale):
    ...
    o = pyredner.load_obj(dir, return_objects=True, device=torch.device("cpu"))
    new_obj = o[0]
  ...
  def rendering(self, cam_pos, cam_up_vector, target_pos, fov, obj_list, size):
    ...
    depth_img = pyredner.render_generic(scene, chan_list,device=torch.device("cpu"))
    near = 0.09
    far = 0.010
  ...
  def getHeightmap(self, objs, object_index, size, scale):
    ...
    tray_dir = "./tray.obj"
    t = pyredner.load_obj(tray_dir, return_objects=True, device=torch.device("cpu"))
    tray = t[0]   
